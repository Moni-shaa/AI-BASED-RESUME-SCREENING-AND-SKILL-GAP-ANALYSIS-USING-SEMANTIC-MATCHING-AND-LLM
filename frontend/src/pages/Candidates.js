  ...
  const filterAndSortCandidates = () => {
    ...
    if (selectedCategory !== 'All') {
      filtered = filtered.filter(
        (candidate) =>
          (selectedCategory === "Qualified" && candidate.shortlisted) ||
          (selectedCategory === "Not a Fit" && !candidate.shortlisted),
      );
    }
            ...
            >
              ...
              <div className="p-6 overflow-y-auto flex-1 prose prose-blue max-w-none">
                {isComparing ? (
                  ...
                  <ReactMarkdown>
                    {comparisonResult || "No comparison data available."}
                  </ReactMarkdown>
                )}
              </div>
            </motion.div>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                <div className="flex items-start">
                  <button
                    ...
                    className={`mt-1 mr-4 p-1 rounded transition-colors ${
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {candidate.analysis?.experience_level} •{" "}
                    ...
                    <div className="flex items-center space-x-2">
                      <span
                        className={`badge ${candidate.shortlisted ? "badge-success" : "badge-danger"}`}
                      >
                        {candidate.shortlisted ? "Qualified" : "Not a Fit"}
                      </span>

                      {candidate.bias_analysis && (
                        <span
                          className={`text-xs font-medium ${getBiasRiskColor(candidate.bias_analysis.overall_bias_score)}`}
                        >
                          {getBiasRiskLabel(
                            candidate.bias_analysis.overall_bias_score,
                          )}
                        </span>
                      )}
                    </div>
